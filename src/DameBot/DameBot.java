...
public class DameBot extends AbstractionLayerAI {
    ...
    private class Ranged {
        ...
        private void assignTask(Unit ranged) {
            ...
            List<Unit> enemyRangedUnitsWithinAttackRange = findUnitsWithin(_ranged, ranged, ranged.getAttackRange());

            if (!enemyRangedUnitsWithinAttackRange.isEmpty()) {
                attack(ranged, findEnemyWithLowestHealth(enemyRangedUnitsWithinAttackRange));
                ...
                attack(ranged, findEnemyWithLowestHealth(enemiesWithinAttackRange));
            } else {
        ...
        private void retreatOrAttack(Unit ranged, List<Unit> enemiesWithinReducedAttackRange,
            ...
            if (bestRetreat != null) {
                ...
                Unit target = findEnemyWithLowestHealth(enemiesWithinAttackRange);
                if (target != null) {
                    attack(ranged, target);
                } else {
                    attack(ranged, findClosest(_units, ranged));
                }
            }
        }
    ...
    private Unit findEnemyWithLowestHealth(List<Unit> enemies) {
    ...
    private double potentialDamage(int x, int y, Unit enemy) {
        int distance = (int) distance(x, y, enemy.getX(), enemy.getY());
        return (distance <= enemy.getAttackRange()) ? enemy.getMaxDamage() : 0;
    }
    ...
    private void attackWithMarch(Unit unit) {
        ...
        Unit enemyBase = findEnemyWithLowestHealth(_bases);
        Unit enemyBarracks = findEnemyWithLowestHealth(_barracks);
        ...
        if (enemyBase != null && enemyBarracks != null) {
            ...
            attack(unit, findEnemyWithLowestHealth(_units));
        }
    }
    ...
    public DameBot(UnitTypeTable unitTypeTable) {
        this(unitTypeTable, new AStarPathFinding());
    }

    public DameBot(UnitTypeTable unitTypeTable, PathFinding pf) {
        super(pf);
    ...
    @Override
    public AI clone() {
        return new DameBot(unitTypeTable, pf);
    }
