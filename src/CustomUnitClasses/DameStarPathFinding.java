 ...
 * A* pathfinding.
 * 
 * The code looks a bit weird, since this version of A* uses static data
 * structures to avoid any
 * memory allocation penalty. It only reallocates memory when asked to
 * path-find for first time,
 * or in a map that is bigger than the previous time.
 * 
 */
public class DameStarPathFinding extends PathFinding {
    ...
    // This function finds the shortest path from 'start' to 'targetpos' and then
    // returns
    // a UnitAction of the type 'actionType' with the direction of the first step in
    // the shortest path
    ...
    /*
     * This function is like the previous one, but doesn't try to reach 'target',
     * but just to
    ...
    public UnitAction findPathToPositionInRange(Unit start, int targetpos, int range, GameState gs, ResourceUsage ru) {
        ...
        int last = pos;
        // System.out.println("- Path from " + start.getX() + "," + start.getY() + " to
        // " + targetpos%w + "," + targetpos/w + " (range " + range + ") in " +
     ...
     * the action to take to walk along the shortest path, or the cost of the
     * shortest
     * path.
     * 
     * @param start
    ...
    private boolean runAStar(Unit start, int targetpos, int range, GameState gs, ResourceUsage ru) {
        ...
        cost[startPos] = 0;
        openinsert++;
        // System.out.println("Looking for path from: " + start.getX() + "," +
        // start.getY() + " to " + targetx + "," + targety);
        while (openinsert > 0) {
             ...
             * System.out.print(" [" + (open[i]%w) + "," + (open[i]/w) + " -> "+
             * cost[open[i]] + "+" + heuristic[i] + "]");
             * }
             * System.out.println("");
             * for(int i = 0;i<h;i++) {
             * for(int j = 0;j<w;j++) {
